...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Enable smooth scrolling in CSS for the whole page
    document.documentElement.style.scrollBehavior = 'smooth';

    // Debounce scroll events to prevent too many calculations
    let scrollTimeout;
    let ticking = false;
    ...
    const handleScroll = () => {
      ...
      // Show/hide nav based on scroll direction with smoother transition
      if (currentScrollY > lastScrollY + 10 && currentScrollY > 99) {
        setShowNav(false);
      ...
      // Calculate progress for animations with less frequent updates
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          const scrollPercentage =
            scrollableDistance > 0 ? currentScrollY / scrollableDistance : 0;
      ...
      scrollTimeout = setTimeout(() => {
        // This will run after scrolling has stopped for 50ms
        if (!isAutoScroll) {
    ...
    // Function to gently snap to the nearest section when scrolling stops
    const snapToNearestSection = () => {
      ...
      // Only snap if we're close enough to a section center
      if (
        smallestDistance < viewportHeight * 0.3 &&
        closestSection !== cubeFace
      ) {
        // Set current section immediately to avoid UI flicker
        setCubeFace(closestSection);

        // Perform the gentle scroll with easing
        if (sectionRefs.current[closestSection]) {
          scrollToSection(closestSection);
        }

        // Show resume only in section 3
        setShowResume(closestSection === 3);
      }
    };
  ...
  }, [lastScrollY, hasMounted, isAutoScroll, cubeFace]);

  // ==================== SECTION SCROLLING FUNCTION ====================
  // Improved section navigation function
  const scrollToSection = (sectionIndex) => {
    if (!hasMounted || !sectionRefs.current[sectionIndex]) return;
    ...
    if (isMobile) {
      ...
      // Desktop behavior with enhanced smoothness
  ...
  // ==================== SECTION TRACKING ====================
  // Section tracking with IntersectionObserver for desktop and optimized calculation for mobile
  useEffect(() => {
    if (!hasMounted) return;

    if (isMobile) {
      // No need for another mobile-specific scroll handler as it's handled in the main scroll effect
      return () => {};
    } else {
